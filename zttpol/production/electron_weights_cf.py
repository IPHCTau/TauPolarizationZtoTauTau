...
@producer(
    uses={
        "Electron.{pt,eta,phi,mass,deltaEtaSC}",
    },
    #produces={
    ...
    weight_name="electron_weight", # new
)
...
def electron_weights_requires(self: Producer,
                              task: law.Task,
                              reqs: dict) -> None:
...
def electron_weights_setup(
        ...
        reqs: dict,
        inputs: dict,
        reader_targets: law.InsertableDict,
) -> None:
    ...
    # check versions
    if self.electron_sf_corrector.version not in (2,3,4,):
        raise Exception(
