...
def enforce_zcand_type(cand, field_type_dict, **kwargs):
    dim = kwargs.get('dim', 'var')
    temp = {}
    ...
    for field, typename in field_type_dict.items():
        ...
        if field not in cand.fields:
            logger.warning(f"{field} not found in enforce_zcand_type: 1 is used instead")
            field_in = ak.ones_like(cand["pt"])
        else:
            field_in = cand[field]
