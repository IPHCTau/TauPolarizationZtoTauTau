...
@calibrator(
    uses={
        "Tau.{pt,eta,phi,mass,charge,decayMode,decayModePNet,genPartFlav}",
        "PuppiMET.{pt,phi}",
    },
    produces={
        "Tau.{pt_no_tes,mass_no_tes}",
        "Tau.{pt,mass}",
        "PuppiMET.{pt_no_tes,phi_no_tes}",
        "PuppiMET.{pt,phi}",
    },
    mc_only=True,
)
def tau_energy_scale(self: Calibrator, events: ak.Array, **kwargs) -> ak.Array:
    ...
    channel = kwargs("channel", None)
    if channel == None:
        raise RuntimeError("One must mention channel name through dedicated calibrator")
    ...
    tes_args = lambda events, mask, deep_tau_tagger, syst: (pt[mask],
                                                            ...
                                                            match[mask],
                                                            deep_tau_tagger,
                                                            self.config_inst.x.deep_tau_info[deep_tau_tagger].vs_j[channel],
                                                            self.config_inst.x.deep_tau_info[deep_tau_tagger].vs_e[channel],
                                                            syst)
    ...
    if n > 0:
        logger.warning(f"Calibration with {channel} WPs : {n} taus with nom tes = 0 for tau pt > 20 GeV")
    ...
    events = set_ak_column_f32(events, "Tau.pt", ak.unflatten(pt * tes_nom, arr_shape))
    events = set_ak_column_f32(events, "Tau.mass", ak.unflatten(mass * tes_nom, arr_shape))
