...
@calibrator(
    uses={"Tau.{pt,eta,phi,mass,charge,genPartFlav,decayMode,decayModePNet}"},
    ...
    get_tau_file=(lambda self, external_files: external_files.tes_sf), # tau_sf (Changed from CP analysis)
...
def tec(
    ...
    *get_tau_file* and *get_tec_config* can be adapted in a subclass in case they are stored
    ...
    dm = events.Tau.decayModePNet
    dm_mask = ((dm >= 0) & (dm != 5) & (dm != 6)) #(dm == 0) | (dm == 1) | (dm == 2) | (dm == 10) | (dm == 11)
...
def tec_init(self: Calibrator, **kwargs) -> None:
    ...
    if self.with_uncertainties:
        ...
        self.produces |= {
            f"{field}_tec_{{jet,e}}_dm{{0,1,2,10,11}}_{{up,down}}"
