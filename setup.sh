...
setup_zttpol() {
    ...
    #   - "ZTTPOL_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "ZTTPOL_" variables is documented below.
    #
    ...
    cf_export_bool ZTTPOL_SETUP
    if ${ZTTPOL_SETUP} && ! ${CF_ON_SLURM}; then
        >&2 echo "the TauPolarizationZtoTauTau analysis was already succesfully setup"
    ...
    #
    # global variables
    # (ZTTPOL = TauPolarizationZtoTauTau, CF = columnflow)
    #

    # start exporting variables
    export ZTTPOL_BASE="${this_dir}"
    export CF_BASE="${cf_base}"
    export CF_REPO_BASE="${ZTTPOL_BASE}"
    export CF_REPO_BASE_ALIAS="ZTTPOL_BASE"
    ...
    if ! ${CF_REMOTE_ENV}; then
        ...
        cf_setup_interactive "${CF_SETUP_NAME}" "${ZTTPOL_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${ZTTPOL_BASE}/bin:${PATH}"
    export PYTHONPATH="${ZTTPOL_BASE}:${ZTTPOL_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${ZTTPOL_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${ZTTPOL_BASE}/modules" ); do
            cf_init_submodule "${ZTTPOL_BASE}" "modules/${m}"
        done
    fi
...
main() {
    ...
    if setup_zttpol "$@"; then
        cf_color green "TauPolarizationZtoTauTau analysis successfully setup"
        return "0"
    else
        local code="$?"
...
# entry point
if [ "${ZTTPOL_SKIP_SETUP}" != "true" ]; then
    main "$@"
fi
