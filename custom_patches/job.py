...
class SlurmJobManager(BaseJobManager):
    ...
    def query(  # type: ignore[override]
        ...
        # Jobs disappear from squeue as soon as they finish.  On clusters where sacct is not
        # reachable from the submission host, retrieve their final state from files written by
        ...
        if missing_ids and status_dir:
            for _job_id in missing_ids:
                ...
                try:
                    ...
                    logger.warning(f"cannot parse Slurm status file '{status_file}': {e}")

            missing_ids = [_job_id for _job_id in missing_ids if _job_id not in query_data]

        # Fall back to sacct unless it was explicitly disabled.  When it is disabled, an absent
        # status file normally means that the batch script has not started writing it yet.
        use_sacct = _cfg.get_expanded_bool("job", "slurm_use_sacct", default=True)
        if missing_ids and not use_sacct:
            for _job_id in missing_ids:
                query_data[_job_id] = self.job_status_dict(
                    job_id=_job_id,
                    status=self.PENDING,
        ...
        # some jobs might already be in the accounting history, so query for remaining job ids
        if missing_ids:
    ...
    def create(
        ...
        **kwargs,
        ...
        def prepare_input(f):
            ...
            # Copy the file into the Slurm submission directory.
            ...
            # provide_input can resolve /home-pbs to /mnt/grid/home-pbs on the
            # submission host. Worker nodes do not expose that resolved path.
            # All copied inputs are located directly inside c.dir, so reconstruct
            # their paths using the logical /home-pbs submission directory.
            return os.path.join(
                os.path.abspath(c.dir),
                os.path.basename(provided_path),
            )
        ...
        for f in c.input_files.values():
            ...
            f.path_job_post_render = (
                f.path_sub_abs
                if f.forward and not f.render_job
        ...
        if c.executable:
            executable_file = c.input_files[executable_key]
            executable_basename = os.path.basename(executable_file.path_sub_abs)
            
            executable_path = os.path.join(c.dir, executable_basename)
            if os.path.exists(executable_path):
                os.chmod(
                    executable_path,
                    os.stat(executable_path).st_mode | stat.S_IXUSR | stat.S_IXGRP,
                )
                
            # Use the logical /home-pbs job directory. Do not use path_sub_abs
            # or SLURM_SUBMIT_DIR because they can resolve to /mnt/grid/home-pbs.
            if executable_file.copy and not executable_file.forward:
                c.executable = shlex.quote(executable_path)
            else:
                c.executable = get_path(executable_file.path_sub_rel)
        ...
        with open(job_file, "w", encoding="utf-8") as f:
            ...
            # Optionally write the job state to shared files.  This provides a status source on
            # clusters where sacct is unavailable from the submission host.
            status_dir = _cfg.get_expanded("job", "slurm_job_status_dir", default=None)
            if status_dir:
                os.makedirs(status_dir, exist_ok=True)
                quoted_status_dir = shlex.quote(status_dir)
                f.write(
                    "\n# Law Slurm status-file fallback\n"
                    "unset LC_ALL\n"
                    "export LANG=C.utf8\n"
